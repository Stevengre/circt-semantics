...
"builtin.module"() ({
  "hw.module"() ({
  ^bb0(%arg0: i3, %arg1: i1, %arg2: !hw.array<1000xi8>):
    ...
    %37 = "hw.constant"() {value = 13 : i10} : () -> i10
    %38 = "hw.array_slice"(%arg2, %37) : (!hw.array<1000xi8>, i10) -> !hw.array<24xi8>
    ...
    %47 = "hw.aggregate_constant"() {fields = [false]} : () -> !hw.uarray<1xi1>
    ...
    "hw.output"(%6) : (i50) -> ()
  }) {module_type = !hw.modty<input arg0 : i3, input arg1 : i1, input arg2 : !hw.array<1000xi8>, output result : i50>, parameters = [], port_locs = [#loc, #loc1, #loc2, #loc3], sym_name = "test1"} : () -> ()
  "hw.module"() ({
  ...
  }) {module_type = !hw.modty<input a : !hw.union<foo: i1, bar: i3>, output x : i3, output z : !hw.union<bar: i3, baz: i8>>, parameters = [], port_locs = [#loc4, #loc5, #loc6], sym_name = "UnionOps"} : () -> ()
  "hw.module"() ({
  ^bb0(%arg0: si8):
    %0 = "sv.wire"() {name = "wireArray"} : () -> !hw.inout<array<2xsi8>>
    %1 = "hw.array_create"(%arg0, %arg0) : (si8, si8) -> !hw.array<2xsi8>
    "sv.assign"(%0, %1) : (!hw.inout<array<2xsi8>>, !hw.array<2xsi8>) -> ()
    %2 = "sv.read_inout"(%0) : (!hw.inout<array<2xsi8>>) -> !hw.array<2xsi8>
    "hw.output"(%2) : (!hw.array<2xsi8>) -> ()
  }) {module_type = !hw.modty<input arg0 : si8, output out : !hw.array<2xsi8>>, parameters = [], port_locs = [#loc7, #loc8], sym_name = "signed_arrays"} : () -> ()
  "hw.module"() ({
  ^bb0(%arg0: !hw.inout<i1>):
    "hw.output"() : () -> ()
  }) {module_type = !hw.modty<inout arg0 : i1>, parameters = [], port_locs = [#loc9], sym_name = "InOutPort"} : () -> ()
  "hw.module"() ({
  ^bb0(%arg0: i32):
    "hw.output"() : () -> ()
  }) {argNames = [""], module_type = !hw.modty<input arg1 : i32>, parameters = [], port_locs = [#loc10], sym_name = "argRenames"} : () -> ()
  "hw.module"() ({
  ^bb0(%arg0: i32):
    "hw.output"() : () -> ()
  }) {module_type = !hw.modty<input arg1 : i32>, output_filelist = #hw.output_filelist<"foo.f">, parameters = [], port_locs = [#loc11], sym_name = "fileListTest"} : () -> ()
  "hw.module"() ({
    "hw.output"() : () -> ()
  }) {comment = "hello world", module_type = !hw.modty<>, parameters = [], sym_name = "commentModule"} : () -> ()
  "hw.module"() ({
  ^bb0(%arg0: i1, %arg1: i1):
    "hw.output"(%arg0, %arg1) : (i1, i1) -> ()
  }) {module_type = !hw.modty<input " d*" : i1, input "" : i1, output "" : i1, output "1" : i1>, parameters = [], port_locs = [#loc12, #loc13, #loc14, #loc15], sym_name = "Foo"} : () -> ()
  "hw.module"() ({
  ^bb0(%arg0: i1):
    %0:2 = "hw.instance"(%arg0, %arg0) {argNames = [" d*", ""], instanceName = "foo", moduleName = @Foo, parameters = [], resultNames = ["", "1"]} : (i1, i1) -> (i1, i1)
    "hw.output"(%0#0, %0#1) : (i1, i1) -> ()
  }) {module_type = !hw.modty<input foo : i1, output "" : i1, output "" : i1>, parameters = [], port_locs = [#loc16, #loc17, #loc18], sym_name = "Bar"} : () -> ()
}) : () -> ()
