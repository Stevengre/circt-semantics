...
def test_diffvcd(now: Path | None = None) -> None:
    ...
    command = ['./scripts/diffvcd.py', '--top1 RocketSystem.RocketSystem.','--top2 TOP.RocketSystem.','-i icache.readEnable','-i icache.writeEnable',str(vcd_file1), str(vcd_file2)]
...
def test_from_setup1(mlir_file: Path, top_module: str, inputs) -> None:
    kcirct = KCIRCT()
    ...
    for input in inputs[394:]:  #从下降沿开始 begin vcd_dump:197
        if 'vcd_dump' in input:
            ...
            kcirct.run_simulate_fast(
                mlir_file.parent / f'setup1.simulated.{run_time}.kore', mlir_file.parent / f'setup1.simulated.{run_time^1}.kore', input_data
            )
            end_time = time.time()
...
def test_evaluate_demo(mlir_file: Path, top_module: str, inputs) -> None:
    ...
    if len(inputs) == 0:
        ...
        for input in inputs[0:]:
            if 'vcd_dump' in input:
                ...
                start_time = time.time()
                kcirct.run_simulate_fast(
                    mlir_file.parent / f'simulated.{run_time}.kore', mlir_file.parent / f'simulated.{run_time^1}.kore', input_data
                )
                end_time = time.time()
                tot_time += end_time - start_time
                run_time = run_time^1
...
def test_from_main(mlir_file: Path, top_module: str, inputs) -> None:
