...
# 解析Kore文本
test_mlir_file = DATA_PATH / 'error_trance_test' / 'test.generic.mlir'
input_kore = DATA_PATH / 'error_trance_test' / 'simulated.0.kore'
err_tracne_file = DATA_PATH / 'error_trance_test' / 'err_trance.json'
test_output = [
    DATA_PATH / 'error_trance_test' / name
    for name in ('test1k.output', 'test1vcd.output', 'testlistk.output', 'testlistvcd.output')
]
test_differences_name = [
    DATA_PATH / 'error_trance_test' / name for name in ('differencesname.txt', 'differencesnameK.txt')
]
...
def test_vcd_list_err_trance(input_list_file: Path, input_kore: Path, err_tracne_file: Path, test_output: Path) -> None:
    k_err_trance = KErrTrance()
    k_err_trance.set_signal_port_mapping(input_kore)
    original_names = []
    ...
    while top < len(k_err_trance.differenes):
        ...
        k_err_trance.search_path_kname(k_err_trance.differenes[top], test_output)
        input()
        top += 1


def test_k_list_err_trance(input_list_file: Path, err_tracne_file: Path, test_output: Path) -> None:
    k_err_trance = KErrTrance()
    ...
    with open(input_list_file, 'r', encoding='utf-8') as file:
        ...
        for line in lines:
            vcdname = line.strip()
            k_err_trance.differenes.append(vcdname)

    k_err_trance.load_from_json(err_tracne_file)

    top = 0
    while top < len(k_err_trance.differenes):
        print(k_err_trance.differenes[top])
        k_err_trance.search_path_kname(k_err_trance.differenes[top], test_output)
        input()
        top += 1
...
def test_err_trance(
    ...
    k_err_trance.search_path_kname(k_target, test_output_k)

    k_err_trance.search_path_vcdname(target, test_output_vcd)
...
if __name__ == '__main__':
    test_evaluate_operation(test_mlir_file, 'Foo', [[(1, 1), (192, 8), (87, 8)]])
    test_build_grapth(input_kore, err_tracne_file)
    ...
    test_err_trance(test_target, test_k_target, input_kore, err_tracne_file, test_output[0], test_output[1])
    test_vcd_list_err_trance(test_differences_name[0], input_kore, err_tracne_file, test_output[3])
    test_k_list_err_trance(test_differences_name[1], err_tracne_file, test_output[2])
