...
"builtin.module"() ({
  "hw.module"() <{module_type = !hw.modty<input a : i8, input b : i8, output res : i8>, parameters = [], result_locs = [#loc], sym_name = "Foo"}> ({
  ^bb0(%arg0: i8, %arg1: i8):
    ...
    "sv.alwayscomb"() ({
      "sv.assert"(%8) <{defer = 0 : i32, label = "overflow_assert"}> : (i1) -> ()
    }) : () -> ()
    "hw.output"(%1) : (i8) -> ()
  }) : () -> ()
}) : () -> ()
