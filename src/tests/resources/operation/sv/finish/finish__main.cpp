...
void getInput(){
    std::ifstream ifs("./sv/finish/test_data.json");
    Value root;
    Reader r;
    r.parse(ifs, root);
    inputs = root["inputs"];
...
int main(int argc, char** argv, char**) {
    ...
    topp = new Vfinish("Foo");
    VerilatedVcdC* tfp = new VerilatedVcdC; // 创建 VCD 对象
    Verilated::traceEverOn(true);
    topp->trace(tfp, 99); // 设置波形跟踪深度
    tfp->open("./sv/finish/trace_vtor.vcd"); // 打开 VCD 文件 ,这里写相对路径的话，我可以在operation目录下
    // Evaluate initials
    topp->eval();  // Evaluate
    ...
    auto seconds = std::chrono::duration_cast<std::chrono::duration<double>>(
                         duration)
                         .count();
    std::cout<<seconds/(main_time+1)<<std::endl;
