...
#include "Vaggregate_constant3.h"
#include <typeinfo>
#include <chrono>
...
void getInput(){
    std::ifstream ifs("./hw/aggregate_constant3/test_data.json");
    Value root;
    Reader r;
    r.parse(ifs, root);
    inputs = root["inputs"];
...
int main(int argc, char** argv, char**) {
    ...
    topp = new Vaggregate_constant3("Foo");
    VerilatedVcdC* tfp = new VerilatedVcdC; // 创建 VCD 对象
    Verilated::traceEverOn(true);
    topp->trace(tfp, 99); // 设置波形跟踪深度
    tfp->open("./hw/aggregate_constant3/trace_vtor.vcd"); // 打开 VCD 文件 ,这里写相对路径的话，我可以在operation目录下
    // Evaluate initials
    topp->eval();  // Evaluate
    ...
    while (!Verilated::gotFinish() && main_time < inputs.size()) {
        // Evaluate model
        topp->a = inputs[int(main_time)][0][0].asLargestUInt();
        topp->b = inputs[int(main_time)][1][0].asLargestUInt();
