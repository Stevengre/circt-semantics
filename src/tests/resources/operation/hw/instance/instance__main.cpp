...
void getInput(){
    std::ifstream ifs("./hw/instance/test_data.json");
    Value root;
    Reader r;
    r.parse(ifs, root);
    inputs = root["inputs"];
...
int main(int argc, char** argv, char**) {
    ...
    topp = new Vinstance("Foo");
    VerilatedVcdC* tfp = new VerilatedVcdC; // 创建 VCD 对象
    Verilated::traceEverOn(true);
    topp->trace(tfp, 99); // 设置波形跟踪深度
    tfp->open("./hw/instance/trace_vtor.vcd"); // 打开 VCD 文件 ,这里写相对路径的话，我可以在operation目录下
    // Evaluate initials
    topp->eval();  // Evaluate
    ...
    while (!Verilated::gotFinish() && main_time < inputs.size()) {
        // Evaluate model
        topp->io_a = inputs[int(main_time)][0][0].asInt();
        topp->eval();
        tfp->dump(main_time);
        ++main_time;
