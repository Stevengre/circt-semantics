...
int main(int argc, char** argv) {
    ...
    Vfirreg_array dut(&context, "Foo");
    VerilatedVcdC trace;
    trace.set_time_unit("1ns");
    trace.set_time_resolution("1ns");
    dut.trace(&trace, 99);
    trace.open("./seq/firreg_array/trace_vtor.vcd");
    std::ifstream source("./seq/firreg_array/test_data.json");
    Json::Value document;
    ...
    for (Json::ArrayIndex t = 0; t < inputs.size(); ++t) {
        ...
        if (!previous_clock && dut.clk) {
            edges = (edges + 1) & 255;
            if (dut.enable) {
                for (unsigned i = 0; i < 4; ++i) expected4[i] = data[i];
            }
            if (dut.reset) {
                expected32 = {0, 20, 0, 54, 0, 88, 0, 122, 0, 156, 0, 190, 0, 224, 0, 258, 0, 292, 0, 326, 0, 360, 0, 394, 0, 428, 0, 462, 0, 496, 0, 530};
            } else if (dut.enable) {
                expected32 = data;
            }
        }
        ...
        if (dut.edges != edges) { std::cerr << "事件 " << t << " 的 edges 不一致: " << unsigned(dut.edges) << " != " << edges << "\n"; trace.close(); return 1; }
    }
    dut.final();
    trace.close();
