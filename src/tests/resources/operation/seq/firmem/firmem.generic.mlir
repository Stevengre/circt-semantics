"builtin.module"() ({
  "hw.module"() ({
    ^bb0(%clk: !seq.clock, %reset: i1, %data_in : i8, %addr : i2, %mode : i1):
      ...
      %read_write_data = "seq.firmem.read_write_port"(%mem, %addr, %clk, %enable, %data_in, %mode) <{operandSegmentSizes = array<i32: 1, 1, 1, 1, 1, 1, 0>}> : (!seq.firmem<4 x 8>, i2, !seq.clock, i1, i8, i1) -> i8

      "hw.output"(%read_data, %read_write_data) : (i8, i8) -> ()
  }) {module_type = !hw.modty<input clk : !seq.clock, input reset : i1,input data_in : i8,input addr : i2,input mode : i1, output read_out : i8, output rw_out : i8>, sym_name = "Foo", parameters = []} : () -> ()
}) : () -> ()
