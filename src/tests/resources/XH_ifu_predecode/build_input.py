...
class PreDecodeInstrGen:
    ...
    def random_instrs(self):
        ...
        # 生成生成 17 个随机的 16 位整数，把相邻的两个 16-bit 数合并成一个 32-bit 数
    ...
    def precoding_checker(self, task='2.1.1'):
        ...
        if task == '2.2.1':
            ...
            def offset(instr):
                ...
                o5 = (offset >> 0) & 1
                offset = (
                    (o11 << 11) | (o4 << 4) | (o98 << 8) | (o10 << 10) | (o6 << 6) | (o7 << 7) | (o31 << 1) | (o5 << 5)
                )
        ...
        if task == '2.2.4':
            ...
            self.instrs = [
                random.getrandbits(5) << 7
                | random.getrandbits(1) << 15
                | 0b1100011
                | random.choice([0b000, 0b001, 0b100, 0b101, 0b110, 0b111]) << 12
                for _ in range(17)
            ]
            self.new_instrs = [self.instrs[i + 1] << 16 | self.instrs[i] for i in range(16)]
            self.jmp_offsets = [offset(i) for i in self.new_instrs]
