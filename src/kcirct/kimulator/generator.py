...
class Generator:
    ...
    def generate(self) -> None:
        ...
        top_module_name = ''
        with open(self._state_json_path) as f:
            # state: [model{name,numStateBytes,states{name,offset,numBits,type}}]
            state: list[StatesJson] = json.load(f)
        ...
        str_context_signals = ''
        str_models = []
        model_signals: dict[str, list[dict[str, str]]] = {}
        model_children: dict[str, list[str]] = {top_module_name.lower() + '_model': []}
        model_names = []
        ...
        for signal in signals.values():
            ...
            str_context_signals += CONTEXT_SIGNAL_TEMPLATE.format(mlir_gen_name=signal.mlir_gen_name, sid=sid)
            ...
            if model_children.get(model_name) is None or model_children[model_name] == []:
                str_models.append(
                    MODEL_TEMPLATE.format(
                        ...
                        model_signals=''.join(
                            [
                                MODEL_SIGNAL_TEMPLATE.format(
                                    name=s['name'], sid=s['sid'], spaces=' ' * (43 + len(model_name))
                                )
    ...
    def generate_json(self) -> subprocess.CompletedProcess:
        ...
        args = ['arcilator', str(self.source), '--state-file=' + str(self._state_json_path), '--observe-ports']
        return subprocess.run(args, stdout=subprocess.PIPE)

    def generate_generic(self) -> subprocess.CompletedProcess:
        """Generate generic MLIR from MLIR"""
        args = ['circt-opt', '--mlir-print-op-generic', str(self.source), '-o', str(self._generic_mlir_path)]
        return subprocess.run(args, stdout=subprocess.PIPE)
