...
def get_cell(term: Pattern, cells: list[tuple[str, CellType]]) -> Pattern | None:
    ...
    if len(cells) == 1:
        if cells[0][1] == CellType.NORMAL:
            return next((cell for cell in term.patterns if cell.symbol == k_symbol(cells[0][0])), None)  # type: ignore
        else:
            return next(
                (cell for cell in term.patterns[0].pattern.patterns if cell.symbol == k_symbol(cells[0][0])), None  # type: ignore
            )
    else:
        return get_cell(get_cell(term, [cells[0]]), cells[1:])  # type: ignore
